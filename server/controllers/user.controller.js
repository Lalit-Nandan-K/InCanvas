...
export const updateUserData = async (req, res) => {
  try {
    ...
    if (tempUser.username !== username) {
      ...
      if (user) {
        // We will not change the username as it is already used by another user
    ...
    //Upload the profile pic and convert into url
    if (profile) {
    ...
    //Upload the cover pic and convert into url
    if (cover) {
...
export const followUser = async (req, res) => {
  try {
    ...
    if (user.following.some((followedId) => followedId.toString() === id.toString())) {
      return res.json({
        success: false,
...
export const unfollowUser = async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: "Now you no longer following this user.",
    });
  } catch (error) {
    console.error(error);
    res.json({ success: false, message: error.message });
